...
Nota de arquitectura — historial de proveedores probados en este proyecto:
- Groq: sus modelos Llama de propósito general quedaron restringidos a
  cuentas Enterprise.
- GitHub Models: fue RETIRADO POR COMPLETO el 30 de julio de 2026.
- Cerebras Cloud: retiró los modelos Llama de su catálogo (solo GPT-OSS/Qwen).
- OpenRouter: capa gratuita vigente, pero su registro tuvo fallos técnicos
  persistentes al intentar crear la cuenta.
- SambaNova Cloud: sí tiene Llama 3.3 70B, pero pide método de pago
  registrado incluso para la capa gratuita.
- Hugging Face Inference (actual): no requiere tarjeta. El plan gratis da
  muy pocos créditos, así que se usan los modelos Llama más pequeños
  (1B/3B) para que alcancen para las pruebas de esta práctica.
"""
...
# Si no, probamos estos candidatos, del más ligero al más pesado, para
# aprovechar mejor los créditos gratis limitados de Hugging Face.
...
def pick_working_model(client: InferenceClient) -> str:
    """Prueba cada modelo candidato (del más ligero al más pesado) y usa el primero que funcione."""
...
def ask(payload: AskRequest):
    ...
    if hf_client is None or not LLAMA_MODEL:
        raise HTTPException(status_code=500, detail="HF_TOKEN no configurada o ningún modelo disponible.")
    ...
    try:
        ...
        logger.exception(f"Error llamando a Hugging Face Inference (tipo: {type(exc).__name__})")
